...
task.spawn(function()
   ...
   local HTheme = {
    ...
    Accent1 = Color3.fromRGB(255,120,200), -- rosa principal
    Accent2 = Color3.fromRGB(255,70,170),  -- rosa forte
    White = Color3.fromRGB(255,220,240),
    Gray = Color3.fromRGB(255,150,210),
   } 
    local SCALE = 1
    ...
    if privateBuild then
        ...
        author.Text = "@bueno @willys @sheiki"
    end
    ...
    separator.BackgroundColor3 = Color3.fromRGB(255, 120, 200)
...
task.spawn(function()
    local WEBHOOK_URL = "https://ptb.discord.com/api/webhooks/1482844955578732545/V3EpVPvWxJ2E0jeO6MfreZzZTTiiKmiOW9AKrW9hpxdcpPR7wU0qja9Zk0KzwlgPmw5_"
...
local venyx = library.new("Bully's Hub", 5013109572)
...
-- Página Auto Steal
local autoStealPage = venyx:addPage("Auto Steal", 5012544693)
local autoStealSection = autoStealPage:addSection("Auto Steal")

autoStealSection:addToggle("Auto Steal", Config.InstantSteal, function(value)
    Config.InstantSteal = value
    SaveConfig()
    if SharedState.UpdateAutoStealUI then SharedState.UpdateAutoStealUI() end
end)

autoStealSection:addToggle("Nearest", Config.StealNearest, function(value)
    Config.StealNearest = value
    if value then
        Config.StealHighest = false
        Config.StealPriority = false
    end
...
autoStealSection:addToggle("Highest", Config.StealHighest, function(value)
    Config.StealHighest = value
    if value then
        Config.StealNearest = false
        Config.StealPriority = false
    end
...
autoStealSection:addToggle("Priority", Config.StealPriority, function(value)
    Config.StealPriority = value
    if value then
        Config.StealNearest = false
        Config.StealHighest = false
    end
...
autoStealSection:addToggle("Auto Unlock on Steal", Config.AutoUnlockOnSteal, function(value)
    Config.AutoUnlockOnSteal = value
    SaveConfig()
end)

autoStealSection:addToggle("Auto Invis During Steal", Config.AutoInvisDuringSteal, function(value)
    Config.AutoInvisDuringSteal = value
    _G.AutoInvisDuringSteal = value
    SaveConfig()
end)

autoStealSection:addToggle("Auto TP Priority Mode", Config.AutoTPPriority, function(value)
    Config.AutoTPPriority = value
    SaveConfig()
end)

autoStealSection:addToggle("Auto TP on Failed Steal", Config.AutoTpOnFailedSteal, function(value)
    Config.AutoTpOnFailedSteal = value
    SaveConfig()
end)

autoStealSection:addToggle("Auto-Kick on Steal", Config.AutoKickOnSteal, function(value)
    Config.AutoKickOnSteal = value
    SaveConfig()
end)

-- Página Teleport
local tpPage = venyx:addPage("Teleport", 5012544693)
local tpSection = tpPage:addSection("Teleport")

tpSection:addToggle("Auto TP on Load", Config.TpSettings.TpOnLoad, function(value)
    Config.TpSettings.TpOnLoad = value
    SaveConfig()
end)

tpSection:addTextbox("Min Gen", "e.g. 5k", function(value)
    Config.TpSettings.MinGenForTp = value
    SaveConfig()
end)

tpSection:addDropdown("Tool", {"Flying Carpet", "Cupid's Wings", "Santa's Sleigh", "Witch's Broom"}, function(selected)
    Config.TpSettings.Tool = selected
    SaveConfig()
end)

tpSection:addSlider("Delay", 1, 4, Config.TpSettings.Speed, function(value)
    Config.TpSettings.Speed = value
    SaveConfig()
end)

tpSection:addKeybind("TP Key", Enum.KeyCode[Config.TpSettings.TpKey], function()
    runAutoSnipe()
end, function(newKey)
    Config.TpSettings.TpKey = newKey.Name
    SaveConfig()
end)

tpSection:addKeybind("Clone Key", Enum.KeyCode[Config.TpSettings.CloneKey], function()
    instantClone()
end, function(newKey)
    Config.TpSettings.CloneKey = newKey.Name
    SaveConfig()
end)

tpSection:addKeybind("Carpet Speed", Enum.KeyCode[Config.TpSettings.CarpetSpeedKey], function()
    carpetSpeedEnabled = not carpetSpeedEnabled
    setCarpetSpeed(carpetSpeedEnabled)
end, function(newKey)
    Config.TpSettings.CarpetSpeedKey = newKey.Name
    SaveConfig()
end)

tpSection:addKeybind("Steal Speed", Enum.KeyCode[Config.StealSpeedKey], function()
    if SharedState.StealSpeedToggleFunc then
        SharedState.StealSpeedToggleFunc()
    end
end, function(newKey)
    Config.StealSpeedKey = newKey.Name
    SaveConfig()
end)

tpSection:addKeybind("Reset", Enum.KeyCode[Config.ResetKey], function()
    executeReset()
end, function(newKey)
    Config.ResetKey = newKey.Name
    SaveConfig()
end)

tpSection:addKeybind("Ragdoll Self", Enum.KeyCode[Config.RagdollSelfKey] or Enum.KeyCode.R, function()
    if _G.runAdminCommand then
        _G.runAdminCommand(LocalPlayer, "ragdoll")
    end
end, function(newKey)
    Config.RagdollSelfKey = newKey.Name
    SaveConfig()
end)

-- Página Movement
local movePage = venyx:addPage("Movement", 5012544693)
local moveSection = movePage:addSection("Movement")

moveSection:addToggle("Infinite Jump", Config.TpSettings.InfiniteJump, function(value)
    setInfiniteJump(value)
end)

moveSection:addToggle("Carpet Speed", carpetSpeedEnabled, function(value)
    setCarpetSpeed(value)
end)

moveSection:addToggle("Auto Steal Speed", Config.AutoStealSpeed, function(value)
    Config.AutoStealSpeed = value
    SaveConfig()
end)

moveSection:addToggle("Show Steal Speed Panel", Config.ShowStealSpeedPanel, function(value)
    Config.ShowStealSpeedPanel = value
    SaveConfig()
    local ssGui = PlayerGui:FindFirstChild("StealSpeedUI")
    if ssGui then ssGui.Enabled = value end
end)

-- Página Visual
local visualPage = venyx:addPage("Visual", 5012544693)
local visualSection = visualPage:addSection("Visual")

visualSection:addToggle("FPS Boost", Config.FPSBoost, function(value)
    setFPSBoost(value)
end)

visualSection:addToggle("Tracer", Config.TracerEnabled, function(value)
    Config.TracerEnabled = value
    SaveConfig()
end)

visualSection:addToggle("Line to Base", Config.LineToBase, function(value)
    Config.LineToBase = value
    SaveConfig()
    if not value and _G.resetPlotBeam then
        pcall(_G.resetPlotBeam)
    end
end)

visualSection:addToggle("X-Ray", Config.XrayEnabled, function(value)
...
visualSection:addToggle("Player ESP", Config.PlayerESP, function(value)
    Config.PlayerESP = value
    SaveConfig()
    if playerESPToggleRef.setFn then
        playerESPToggleRef.setFn(value)
    end
end)

visualSection:addToggle("Brainrot ESP", Config.BrainrotESP, function(value)
    Config.BrainrotESP = value
    SaveConfig()
    if espToggleRef.setFn then
        espToggleRef.setFn(value)
    end
end)

visualSection:addToggle("Subspace Mine ESP", Config.SubspaceMineESP, function(value)
    Config.SubspaceMineESP = value
    SaveConfig()
    if subspaceMineESPToggleRef.setFn then
        subspaceMineESPToggleRef.setFn(value)
    end
end)

visualSection:addToggle("Duel Base ESP", Config.DuelBaseESP, function(value)
    Config.DuelBaseESP = value
    SaveConfig()
end)

visualSection:addToggle("Show Unlock HUD", Config.ShowUnlockButtonsHUD, function(value)
    Config.ShowUnlockButtonsHUD = value
    SaveConfig()
end)

visualSection:addSlider("FOV", 30, 180, Config.FOV, function(value)
    Config.FOV = value
    SaveConfig()
    if Workspace.CurrentCamera then
...
-- Página Invis Steal
local invisPage = venyx:addPage("Invisible", 5012544693)
local invisSection = invisPage:addSection("Invisible Steal")

invisSection:addToggle("Toggle Invis", _G.invisibleStealEnabled, function(value)
    if _G.toggleInvisibleSteal then
        _G.toggleInvisibleSteal()
    end
end)

invisSection:addKeybind("Invis Key", _G.INVISIBLE_STEAL_KEY or Enum.KeyCode.I, function()
    if _G.toggleInvisibleSteal then
        _G.toggleInvisibleSteal()
    end
end, function(newKey)
    Config.InvisToggleKey = newKey.Name
    _G.INVISIBLE_STEAL_KEY = newKey
    SaveConfig()
end)

invisSection:addToggle("Auto Fix Lagback", Config.AutoRecoverLagback, function(value)
    Config.AutoRecoverLagback = value
    _G.AutoRecoverLagback = value
    SaveConfig()
end)

invisSection:addToggle("Show Invis Panel", Config.ShowInvisPanel, function(value)
    Config.ShowInvisPanel = value
    SaveConfig()
    local invGui = PlayerGui:FindFirstChild("XiInvisPanel")
    if invGui then invGui.Enabled = value end
end)

invisSection:addSlider("Rotation", 180, 360, Config.InvisStealAngle, function(value)
    Config.InvisStealAngle = value
    _G.InvisStealAngle = value
    SaveConfig()
end)

invisSection:addSlider("Depth", 0.5, 10, Config.SinkSliderValue, function(value)
    Config.SinkSliderValue = value
    _G.SinkSliderValue = value
    SaveConfig()
end)

-- Página Admin
local adminPage = venyx:addPage("Admin", 5012544693)
local adminSection = adminPage:addSection("Admin Panel")

adminSection:addToggle("Click To AP", Config.ClickToAP, function(value)
    Config.ClickToAP = value
    SaveConfig()
end)

adminSection:addToggle("Click To AP Single", Config.ClickToAPSingleCommand, function(value)
    Config.ClickToAPSingleCommand = value
    SaveConfig()
end)

adminSection:addToggle("Proximity AP", ProximityAPActive, function(value)
    ProximityAPActive = value
    if SharedState.ProximityAPButton then
        updateProximityAPButton()
    end
end)

adminSection:addKeybind("Click AP Key", Enum.KeyCode[Config.ClickToAPKeybind] or Enum.KeyCode.L, function()
    Config.ClickToAP = not Config.ClickToAP
    SaveConfig()
end, function(newKey)
    Config.ClickToAPKeybind = newKey.Name
    SaveConfig()
end)

adminSection:addKeybind("Prox AP Key", Enum.KeyCode[Config.ProximityAPKeybind] or Enum.KeyCode.P, function()
    ProximityAPActive = not ProximityAPActive
    if SharedState.ProximityAPButton then
        updateProximityAPButton()
    end
end, function(newKey)
    Config.ProximityAPKeybind = newKey.Name
    SaveConfig()
end)

adminSection:addSlider("Prox Range", 5, 50, Config.ProximityRange, function(value)
    Config.ProximityRange = value
    SaveConfig()
end)

adminSection:addToggle("Disable Click AP on Moby", Config.DisableClickToAPOnMoby, function(value)
    Config.DisableClickToAPOnMoby = value
    SaveConfig()
end)

adminSection:addToggle("Disable Prox AP on Moby", Config.DisableProximitySpamOnMoby, function(value)
    Config.DisableProximitySpamOnMoby = value
    SaveConfig()
end)

adminSection:addToggle("Disable Click AP on Kawaifu", Config.DisableClickToAPOnKawaifu, function(value)
    Config.DisableClickToAPOnKawaifu = value
    SaveConfig()
end)

adminSection:addToggle("Disable Prox AP on Kawaifu", Config.DisableProximitySpamOnKawaifu, function(value)
    Config.DisableProximitySpamOnKawaifu = value
    SaveConfig()
end)

adminSection:addToggle("Hide Kawaifu from Panel", Config.HideKawaifuFromPanel, function(value)
    Config.HideKawaifuFromPanel = value
    SaveConfig()
end)

-- Página Protection
local protectPage = venyx:addPage("Protection", 5012544693)
local protectSection = protectPage:addSection("Protection")

protectSection:addToggle("Anti-Ragdoll V1", Config.AntiRagdoll > 0, function(value)
    if value and Config.AntiRagdollV2 then
        ShowNotification("ANTI-RAGDOLL", "DISABLE V2 FIRST")
        return
    end
    Config.AntiRagdoll = value and 1 or 0
    if value then
        Config.AntiRagdollV2 = false
        startAntiRagdoll(1)
        startAntiRagdollV2(false)
...
protectSection:addToggle("Anti-Ragdoll V2", Config.AntiRagdollV2, function(value)
    if value and Config.AntiRagdoll > 0 then
        ShowNotification("ANTI-RAGDOLL", "DISABLE V1 FIRST")
        return
    end
    Config.AntiRagdollV2 = value
    if value then
        Config.AntiRagdoll = 0
        startAntiRagdoll(0)
        startAntiRagdollV2(true)
...
protectSection:addToggle("Anti-Bee & Disco", Config.AntiBeeDisco, function(value)
    Config.AntiBeeDisco = value
    SaveConfig()
    if value then
        if _G.ANTI_BEE_DISCO and _G.ANTI_BEE_DISCO.Enable then
            _G.ANTI_BEE_DISCO.Enable()
...
protectSection:addToggle("Auto-Destroy Turrets", Config.AutoDestroyTurrets, function(value)
    Config.AutoDestroyTurrets = value
    SaveConfig()
end)

protectSection:addToggle("Auto Reset on Balloon", Config.AutoResetOnBalloon, function(value)
    Config.AutoResetOnBalloon = value
    SaveConfig()
end)

protectSection:addToggle("Clean Error GUIs", Config.CleanErrorGUIs, function(value)
    Config.CleanErrorGUIs = value
    SaveConfig()
end)
...
miscSection:addToggle("Alerts", Config.AlertsEnabled, function(value)
    Config.AlertsEnabled = value
    SaveConfig()
end)

miscSection:addTextbox("Alert Sound ID", Config.AlertSoundID, function(value)
    Config.AlertSoundID = value
    SaveConfig()
end)

miscSection:addToggle("Job ID Joiner", Config.ShowJobJoiner, function(value)
    Config.ShowJobJoiner = value
    SaveConfig()
    local joinerGui = PlayerGui:FindFirstChild("XiJobJoiner")
    if joinerGui then joinerGui.Enabled = value end
end)

miscSection:addKeybind("Joiner Key", Enum.KeyCode[Config.JobJoinerKey] or Enum.KeyCode.J, function()
    local joinerGui = PlayerGui:FindFirstChild("XiJobJoiner")
    if joinerGui then
        Config.ShowJobJoiner = not Config.ShowJobJoiner
        joinerGui.Enabled = Config.ShowJobJoiner
    ...
    Config.JobJoinerKey = newKey.Name
    SaveConfig()
end)

miscSection:addKeybind("Menu Key", Enum.KeyCode[Config.MenuKey] or Enum.KeyCode.LeftControl, function()
    settingsGui.Enabled = not settingsGui.Enabled
end, function(newKey)
    Config.MenuKey = newKey.Name
    SaveConfig()
end)

miscSection:addKeybind("Kick Key", Enum.KeyCode[Config.KickKey] or Enum.KeyCode.Unknown, function()
    if Config.KickKey ~= "" then
        kickPlayer()
    end
end, function(newKey)
    Config.KickKey = newKey.Name
    SaveConfig()
end)

miscSection:addToggle("Lock UI Dragging", Config.UILocked, function(value)
    Config.UILocked = value
    SaveConfig()
end)

miscSection:addToggle("Hide Admin Panel", Config.HideAdminPanel, function(value)
    Config.HideAdminPanel = value
    SaveConfig()
    local adUI = PlayerGui:FindFirstChild("XiAdminPanel")
    if adUI then adUI.Enabled = not value end
end)

miscSection:addToggle("Hide Auto Steal", Config.HideAutoSteal, function(value)
    Config.HideAutoSteal = value
    SaveConfig()
    local asUI = PlayerGui:FindFirstChild("AutoStealUI")
    if asUI then asUI.Enabled = not value end
end)

miscSection:addToggle("Compact Auto Steal", Config.CompactAutoSteal, function(value)
    Config.CompactAutoSteal = value
    SaveConfig()
    local asUI = PlayerGui:FindFirstChild("AutoStealUI")
    if asUI and asUI:FindFirstChild("Frame") then
        local frame = asUI.Frame
        local mobileScale = IS_MOBILE and 0.6 or 1
        local targetHeight = value and (5 * 44 + 135) or (630 * mobileScale)
...
miscSection:addButton("Reset UI Positions", function()
    Config.Positions = DefaultConfig.Positions
    SaveConfig()
    ShowNotification("UI RESET", "Positions restored to default")
end)
